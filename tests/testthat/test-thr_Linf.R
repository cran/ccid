context("The change-points in the mean structure for a multivariate
        time series of multiplicative structure via taking the maximum of
        contrast function values")

# In this file we test the thr_Linf function
...
test_that("The correct change-points are given when the function is used correctly", {
   ...
   expect_equal(thr_Linf(t(hdbinseg::gen.input(t(X), -1, TRUE, diag = FALSE, sgn))), c(100))
})
...
test_that("Error and warning messages are given correctly", {
  ...
  expect_error(thr_Linf(X = "I am not a numeric matrix"))

  # The threshold constant is set equal to zero.
  expect_error(thr_Linf(X = test_data, thr_const = 0))

  # The value for lambda is negative.
  expect_error(thr_Linf(X = test_data, points = -2))

  # The value for lambda is a positive real number instead of a positive integer.
  expect_warning(thr_Linf(X = test_data, points = 3.4))
})
